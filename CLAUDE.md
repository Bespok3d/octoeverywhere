# octoeverywhere-plugin: instructions for AI assistants
...
This is a solo repo publishing a single atom into `Bespok3d/main-index`.

- `octoeverywhere`: runs the OctoEverywhere Moonraker companion for free remote access and AI failure
  detection.

It carries Python: the companion app and its dependencies are baked into the package, so the printer
provisions a per-plugin venv offline and never runs pip or git.
